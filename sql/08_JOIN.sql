...
-- JOIN 방법
-- CROSS JOIN
-- * Equi JOIN(등가조인,내부조인,네추랄조인)
-- Non Equi JOIN <- 거의사용하지 않음
...
-- CROSS 와 JOIN 는 생략 가능
SELECT e.ename,d.deptno,d.deptname FROM emp e, dept d;

-- 1. Equi JOIN
-- CROSS JOIN 에서 두 테이블이 동일하게 있는 값만을 추출한 형태
...
-- USING 을 사용하면 조인에 사용할 컬럼이나 뷰 서브쿼리 등을 사용할수 있다.
...
-- 외부조인
-- 두 테이블 중 보여줄 종류가 더 많은 테이블을 지목한는 형태
SELECT e.ename,d.deptno,d.deptname 
	FROM emp e RIGHT OUTER JOIN dept d ON e.deptno = d.deptno;

-- dept 에는 없고 emp 에만 있는 deptno 를 넣으려고 한다.
-- dept 는 emp 의 부모이기 때문에 emp 에 없는 deptno 란 있을수 없다.
