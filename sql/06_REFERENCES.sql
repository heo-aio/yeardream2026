...
CREATE TABLE iden_table(
	...
	price INT(10),
	qty VARCHAR(5),
	FOREGIN KEY(user_id) REFERENCES parent_table(user_id)
);
DESC iden_table;
...
-- 이경우 자식을 먼저 다 지우고, 이후에 부모를 지워야 한다.(간단하게 할 수 있는 방법 -> 마지막에)
...
-- 특정한 조간이 맞을 경우 입력시켜 준다.
-- ATER TABLE [테이블명] ADD CONSTRAINT CHECK(조건)
...
INSERT INTO chlid_table VALUES(4,'user02','신발',10000,1);
